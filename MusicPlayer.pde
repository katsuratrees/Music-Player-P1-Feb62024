...
PImage backgroundImage, backgroundImagePath;
int size;
PFont generalFont;
String quit="QUIT";
//
...
String road = "road";
String Lights = "ltotbngnzzu-uai-1600x900";
...
rect(backgroundImageX, backgroundImageY, backgroundImageWidth, backgroundImageHeight);
  //
    String backgroundImagePath = sketchPath ( pathway + road + ImageExtension );
backgroundImage = loadImage( backgroundImagePath );
  ...
  if ( dayMode==true && hour()>=9 && hour()<=17 ) {
    ...
    path = pathway + road + ImageExtension;
...
void draw() {
  ...
  if ( lightMode == true ) { //Boolean keyBind
    // //Caution: See setup
    //    backgroundImageName = ;
    path = sketchPath ( pathway + road + ImageExtension);
    backgroundImage = loadImage( path );
  } else {
    // backgroundImageName = ;
    path1 = pathway + Lights + ImageExtension;
